...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    // ======================
    // Usa a variável de ambiente para obter o ID do canal
    const practiceChannelId = YESTERDAY_PRACTICE_CHANNEL_ID;
    const channel = client.channels.cache.get(practiceChannelId);

    if (!channel) {
      return interaction.reply({
        content: "Canal de prática não encontrado! Por favor, configure o ID no arquivo .env.",
        ephemeral: true
      });
    }
    ...
    collector.on("end", async (collected, reason) => {
      ...
      if (reason === "correct") {
        ...
        interaction.followUp({
          content: "⏰ Time's up! Use `/yesterday` to try again.",
          ephemeral: true
        });
        client.currentQuestion[userId] = null;
      }
